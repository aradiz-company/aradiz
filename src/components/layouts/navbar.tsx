        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          <SheetTrigger asChild className="lg:hidden">
            <Button
              variant="ghost"
              size="icon"
              className="h-10 w-10 rounded-full border border-foreground/60 bg-transparent text-foreground hover:bg-black/5"
            >
              {isOpen ? (
                <X className="h-7 w-7" />
              ) : (
          ...
          <SheetContent
            side="top"
            className={`${isScrolled ? "top-16" : "top-20"} h-min max-h-[calc(100dvh-4rem)] overflow-y-auto border-t border-border/10 bg-background/60 p-0 shadow-none backdrop-blur-md data-[state=closed]:duration-500 data-[state=open]:duration-700`}
                  ...
                  {siteConfig.navigation.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        className={`flex items-center justify-between text-xl font-light tracking-tight transition-all duration-300 group ${
                          isActive
                      ...
                      >
                        {item.name}
                        <span
                          className={`h-0.5 w-0 bg-primary transition-all duration-300 group-hover:w-8 ${isActive ? "w-8" : ""}`}
                        ></span>
                      </Link>
                    );
                  })}
